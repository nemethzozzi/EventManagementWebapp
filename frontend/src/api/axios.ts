...
apiClient.interceptors.response.use(
    ...
    async (error) => {
        ...
        if (!error.response) {
            notify.error('axios.error.server_unavailable')
            return Promise.reject(error)
        }
        ...
        if (status === 401 && !originalRequest._retry && !isLoginRequest && !isRefreshRequest) {
            ...
            try {
                ...
                // Refresh sikertelen: 1 toast + logout
                notify.error('axios.error.expired_session')
        ...
        // 403 jogosultság
        if (status === 403) {
            notify.error('axios.error.forbidden')
            return Promise.reject(error)
        }

        // 5xx szerverhiba
        if (status >= 500) {
            notify.error('axios.error.server_error')
            return Promise.reject(error)
        }

        // Egyéb 4xx hibák: ha van backend üzenet, mutassuk
        if (backendMessage) {
            notify.error(String(backendMessage))
        }
