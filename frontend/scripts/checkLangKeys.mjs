...
const KEY_REGEXES = [
  ...
  /trans\(\s*['"`]([^'"`]+?)['"`]\s*[\),]/g,
  /Lang::get\(\s*['"`]([^'"`]+?)['"`]\s*[\),]/g,
  /['"`](message_key|error_key)['"`]\s*=>\s*['"`]([^'"`]+?)['"`]/g, // PHP array style
  /['"`](message_key|error_key)['"`]\s*:\s*['"`]([^'"`]+?)['"`]/g, // JS/JSON object style
]
...
function loadLocaleKeys() {
  ...
  const files = fs.readdirSync(FRONTEND_LOCALES_DIR).filter((f) => f.endsWith('.json'))
...
function main() {
  ...
  if (missing.length) {
    printList(red('USED IN CODE BUT MISSING IN JSON'), missing, (s) => red(s))
  }

  if (unused.length) {
    printList(yellow('PRESENT IN JSON BUT UNUSED IN CODE'), unused, (s) => yellow(s))
  }
